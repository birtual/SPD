       	...
       	sql+= " order by apartado desc, prioridad asc, orden asc  ";
		    ...
		    try {
		    	...
		    	while (resultSet.next()) {
		    		...
		    		//si la última ejecutó no tiene fecha la incorporamos como activa
...
/*
	...
	/** ok
	 * Método que se encarga de construir un bean a partir de un registro de procesoejecutó de la bbdd
	 * @param rs
	 * @return
	 * @throws SQLException
	 */
...
*//*
		public void resetearProcesosEnEjecucion() throws SQLException {
			...
			Connection con = null;
			con = Conexion.conectar();
			    String sql = "UPDATE SPD_procesos SET estado = 'No iniciado' WHERE estado = 'En ejecutó'";
			    try (PreparedStatement ps = con.prepareStatement(sql)) {
			        ps.executeUpdate();
			    }
			}
