...
public class SustXComposicionDAO {
	...
	public static List<SustXComposicion> getSustXComposicion(SustXComposicionForm form, int inicio, int fin) throws ClassNotFoundException, SQLException {
		...
		String qry = getQueryPartsSustituciones( form, false, inicio, fin, true);
	...
	public static  boolean desAsignacionMasiva(SustXComposicionForm form) throws ClassNotFoundException, SQLException
	{
		...
		if(form.isFiltroCheckedLabsSoloAsignados())
			query+=  " and codiLab =  '"+form.getFiltroCodiLaboratorio() +"' ";
		//if(form.getFiltroCodGtVmp()!=null && !form.getFiltroCodGtVmp().equals(""))
		//	query+=  " and codGtVmp =  '"+form.getFiltroCodGtVmp() +"' ";
		//if(form.getFiltroListaCodGtVm()!=null && !form.getFiltroListaCodGtVm().equals(""))
		//	query+=  " and codGtVm in (select codGtVmp from bd_consejo where codGtVm like  '%"+form.getFiltroListaCodGtVm() +"%' )";
		//if(form.getFiltroCodGtVmp()!=null && !form.getFiltroCodGtVmp().equals(""))
		//	query+=  " and codGtVmp in (select codGtVmp from bd_consejo where codGtVmp  =  '"+form.getFiltroCodGtVmp() +"'  )";
	...
	private static String getQueryPartsSustituciones(SustXComposicionForm form, boolean count, int inicio, int fin, boolean todosLosConjuntosHomogeneos) {
		...
		String order = 	getOrder(form, count);
		 order = 	"";
		String otros = 	getOtrosSql2008(form, inicio, fin, count);
	
		String qry = select + from + where + order + otros;
		System.out.println("SustXComposicionDAO getQueryPartsSustituciones -->" +qry );		
		return qry;
	}


	private static  String getSelect(SustXComposicionForm form, boolean count) 
	{
		String select = "";
		//si es contador inicializo la query
		if(count)  
			...
			select = "select * from ( select  distinct ROW_NUMBER() OVER(ORDER BY g.nomGtVmpp, (g.rentabilidad + g.ponderacion) desc) AS ROWNUM, g.idrobot, b.CodGtVmp,  b.NomGtVmp, g.sustituible, g.tolva, "; // grupo VMP (igual ConjHomog - principio activo, dosis y forma farmacéutica).
		...
		select+=" b.CodGtAtcNivel4, b.NomGtAtcNivel4, 	";		// nivel de grupo terapéutico de la ATC (mismo subgrupo farmacológico/terapéutico).
		select+=" b.CodGtAtcNivel5, b.NomGtAtcNivel5, 	";		// nivel de grupo terapéutico de la ATC (mismo principio activo)
		select+=" coalesce(b.CodGtVmpp, g.CodGtVmpp) as codGtVmpp, coalesce(b.NomGtVmpp, g.NomGtVmpp) as nomGtVmpp, ";  				// grupo VMMP (igual principio activo, dosis, forma farmacéutica y número de unidades de dosificación).
		select+=" g.rentabilidad, g.ponderacion, g.codiLab, g.ultimaModificacion, (g.rentabilidad + g.ponderacion) as nota,  ";
	...
	private static  String getFrom(SustXComposicionForm form)
	{
		String from =" from dbo.SPD_sustXComposicion g left join bd_consejo b on ( g.codGtVmpp=b.CodGtVmpp and g.codiLab=b.codiLabo and g.CN6=b.codigo )   ";
		//String from =" from dbo.SPD_sustXComposicion g full outer join bd_consejo b on (b.codiLABO =g.codiLAB  and g.codConjHomog=b.codConjHomog and g.codConjHomog>0)  ";
		//String from =" from dbo.SPD_sustXComposicion g left join bd_consejo b on (g.codConjHomog=b.codConjHomog and g.codConjHomog>0)  ";
		/*if(form.isFiltroCheckedLabsSoloAsignados())
		{
	...
	private static  String getWhere(SustXComposicionForm form) 
	{
		String 	where =" where 1=1  ";
		where+=" and g.codGtVmpp is not null ";
		...
		{
			where+=  " and ( ";
			where+=  "  ( g.nomGtVmpp like '%"+form.getFiltroTextoABuscar() +"%' OR b.nomGtVm like '%"+form.getFiltroTextoABuscar() +"%' )";
			where+=  "  OR g.nombreLab  like '%"+form.getFiltroTextoABuscar() +"%' ";
			where+=  "  OR g.comentarios  like '%"+form.getFiltroTextoABuscar() +"%' ";
			where+=  "  OR b.CODIGO ='"+form.getFiltroTextoABuscar() +"'";
	// 		where+=  "  OR b.NOMBRE like '%"+form.getFiltroTextoABuscar() +"%' ";			Da problemas. Muestra más registros de la cuenta
	//		where+=  "  OR b.PRESENTACION like '%"+form.getFiltroTextoABuscar() +"%' ";		Da problemas. Muestra más registros de la cuenta
			where+=  " ) ";
		}
		if(form.getFiltroRobot()!=null && !form.getFiltroRobot().equals(""))
			where+=  " and g.idRobot =  '"+form.getFiltroRobot() +"' ";
		if(form.getFiltroCodiLaboratorio()!=null && !form.getFiltroCodiLaboratorio().equals(""))
			where+=  " and (b.codiLABO =  '"+form.getFiltroCodiLaboratorio() +"' OR g.codiLab =  '"+form.getFiltroCodiLaboratorio() +"') ";
		if(form.getFiltroCodGtVm()!=null && !form.getFiltroCodGtVm().equals(""))
			where+=  " and b.CodGtVm =  '"+form.getFiltroCodGtVm() +"' ";
		if(form.getFiltroCodGtVmp()!=null && !form.getFiltroCodGtVmp().equals(""))
			where+=  " and b.codGtVmp =  '"+form.getFiltroCodGtVmp() +"'  ";
		if(form.getFiltroCodGtVmpp()!=null && !form.getFiltroCodGtVmpp().equals(""))
			where+=  " and g.codGtVmpp =  '"+form.getFiltroCodGtVmpp() +"'  ";
		if(form.getFiltroPresentacion()!=null && !form.getFiltroPresentacion().equals(""))
			where+=  " and b.presentacion like  '%"+form.getFiltroPresentacion() +"%' ";
		if(form.getFiltroCodigoMedicamento()!=null && !form.getFiltroCodigoMedicamento().equals(""))
			where+=  " and b.CODIGO like  '%"+form.getFiltroCodigoMedicamento() +"%' ";
		...
		if(form.getNombreLab()!=null && !form.getNombreLab().equals(""))
			where+=  " and b.nomLABO like  '%"+form.getNombreLab() +"%' ";	
		if(form.isFiltroCheckedLabsSoloAsignados() || form.isFiltroLabsSoloAsignados())
		{
			//where+=  " and g.codiLab =  '"+form.getFiltroCodiLaboratorio() +"' ";
			where+=  " and g.codiLab is not null ";
			if(form.getCodiLab()!=null && !form.getCodiLab().equals(""))		
				where+=  " g.codiLab =  '"+form.getCodiLab() +"') ";
			if(form.getFiltroCodiLaboratorio()!=null && !form.getFiltroCodiLaboratorio().equals(""))
				where+=  " and (g.codiLab =  '"+form.getFiltroCodiLaboratorio() +"') ";
		}
		else
		...
		if(form.isFiltroCheckedComposicionSinLabs() || form.isFiltroComposicionSinLabs())
			where+=  " and b.codGtVmpp not in (select codGtVmpp from SPD_sustXComposicion)  ";
		return where;
	}

	private static String getOrder(SustXComposicionForm form, boolean count) 
	{
		String order="";
		if(!count) 
		{
			order+=  " order by b.nomGtVmpp, nota desc, g.ponderacion ";
		}
		return order;
	}
	 ...
	 * @return
	 */
	private static String getOtrosSql2008(SustXComposicionForm form, int inicio, int fin, boolean count) 
	{
		String otros="";
		if(!count) 
		{
