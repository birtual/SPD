...
public class SustXGtDAO2 {
	...
	*/
	...
	private static Nivel3 creaGtVmpp(ResultSet resultSet) throws SQLException {
		...
		hijo.setOidSustXComposicion(resultSet.getInt("oidSustXComposicion"));
	...
	private static  String getSelect(SustXGtForm form, boolean count) 
	{
		...
		else if(!count) 
		{
			...
			select+= " COALESCE(g.cn6, b.codigo, '') AS cn6,  ";
			select+= " COALESCE(g.cn7, dbo.calcularCN7(b.codigo), '') AS cn7, ";
			...
			if(form.getFiltroCodGtVmp()!=null && !form.getFiltroCodGtVmp().equalsIgnoreCase(""))
				...
				//INICIO OR - en caso de OR abrimos un parántesis antes de los tres filtros
			if(esOR)
			{
				select+=  " AND (  1 = 0 ";
			}
			...
			//FIN OR - en caso de OR cerramos un parántesis antes de los tres filtros
			if(esOR)
			{
				select+=  " )";
			}
			...
			if(count)  
				...
				select+= " SELECT COUNT(distinct coalesce(b2.CodGtVm,'')) AS total ";
			else if(!count) 
			{
				select+= " SELECT DISTINCT "; 
				select+= " 	b2.CodGtVm AS CodGtVm, ";
				select+= " 	b2.NomGtVm AS NomGtVm, ";
			//	select+= "  b2.presentacion, ";
			...
			select+= " WHERE 1=1   AND b2.NomGtVmp<>''  ";
			if(form.getOidSustXComposicion()>0)
